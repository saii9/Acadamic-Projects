...
# file name     : project3.py - THIS PROGRAM IS WITH MULTI THREADING
...
def checkForDomainInString(request):
    ...
    #defining list to hold thread objects
thread_list = []
srcs_to_replace = dict()

#defining a thead that will be used for multi threading
def objectDownloadThread(homepage, initreq, host, folderToSave):
    try:
        request = initreq
        ...
        if initreq.rfind("/") != -1:
            ...
            writeFile(folderToSave, initreq, body)
    except:
        None
...
def processReqArray(homepage, reqArray, host, folderToSave):
    ...
    for initreq in reqArray:
        t = threading.Thread(target=objectDownloadThread, args=(homepage, initreq, host, folderToSave))
    ...
    for i in range (0, len(thread_list)):
        thread_list[i].join()
        sys.stdout.write("\rThread Joined for Object: %d" %(i+1))
        sys.stdout.flush()
...
def main():
    print "downloading site with Multi threading:"+sys.argv[1]
    parser = MyHTMLParser()
    try:
