function pathplan
    ...
    L(1) = Revolute('d', 0,     'a', 0.109, 'alpha', 0, ...
        'qlim', [129 -51] );
...
for i=1:4
    ...
    elseif i==3
        clear p;
        disp ('with mstraj');
        p = mstraj(pt(2:length(pt)-1,1:2), [pt(1,1),pt(1,2)], [], [pt(length(pt),1),pt(length(pt),2)],0.05,.5);
        close(figure(1));
        readmap(p,pt);
        x = input('Hit enter to write');
    elseif i==4
...
%     a.servoWrite(1,-1*round(j(1,1)*180/pi) + 134);      
%     a.servoWrite(2,-1*round(j(1,2)*180/pi) - 32);
